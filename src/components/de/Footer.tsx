...
import logo from "@/assets/de/aidex-logo.png";
import { deBrand } from "@/lib/locale/de/brand";
import { DE_FREE_SHIPPING_LABEL } from "@/lib/locale/de/bundles";
import { dePaths } from "@/lib/locale/de/paths";

const productLinks = [
  { label: "AiDEX G7", to: dePaths.product },
  { label: "Pläne", href: `${dePaths.home}#plaene` },
  { label: "So funktioniert's", href: `${dePaths.home}#technologie` },
  { label: "Spezifikationen", to: dePaths.product },
] as const;

const supportLinks = [
  { label: "FAQ", to: dePaths.faq },
  { label: "Sendung verfolgen", to: dePaths.tracking },
  { label: "Kontakt", to: dePaths.contact },
  { label: "Über uns", to: dePaths.about },
] as const;

const legalLinks = [
  { label: "Versand", to: dePaths.shipping },
  { label: "Datenschutz", to: dePaths.privacy },
  { label: "Rückerstattung", to: dePaths.refund },
] as const;

const highlights = [
  { Icon: Truck, label: "Kostenloser Versand" },
  { Icon: Activity, label: "24h Monitoring" },
  { Icon: Smartphone, label: "App auf Deutsch" },
  { Icon: Droplets, label: "IP68" },
  { Icon: ShieldCheck, label: "SSL sicher" },
  { Icon: Package, label: "Sendungsverfolgung" },
] as const;
    ...
    <footer className="bg-[var(--ink)] text-white">
      <div className="border-b border-white/10 bg-[var(--primary-deep)]">
        <div className="container-edge py-6 md:py-7 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <p className="font-display text-xl md:text-2xl leading-tight text-balance">
            Kontinuierliches Glukose-Monitoring in Echtzeit.
          </p>
          <Link
            to={dePaths.product}
      ...
      <div className="container-edge py-8 md:py-10">
        <div className="grid grid-cols-2 md:grid-cols-12 gap-8 md:gap-6 lg:gap-8">
          <div className="col-span-2 md:col-span-4">
            <Link to={dePaths.home} aria-label="AiDEX — Startseite" className="inline-block group">
            ...
            <p className="mt-4 max-w-xs text-sm text-white/60 leading-relaxed">
              {deBrand.productName} · {deBrand.tagline}. {deBrand.manufacturer}.
            </p>
            <a
              href={`mailto:${deBrand.email}`}
          ...
          <div className="col-span-2 md:col-span-4">
            ...
            <p className="mt-4 text-[10px] leading-relaxed text-white/35 md:max-w-xs">
              CGM-Gerät. Konsultieren Sie Ihren Arzt. {DE_FREE_SHIPPING_LABEL}.
            </p>
          </div>
        </div>
      ...
      <div className="border-t border-white/10">
        <div className="container-edge py-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 text-[10px] uppercase tracking-[0.14em] text-white/40">
          <span>
            © {year} {deBrand.name} · {deBrand.manufacturer}
          </span>
          <div className="flex flex-wrap gap-x-5 gap-y-1">
            <Link to={dePaths.privacy} className="hover:text-[var(--primary)]">
              Datenschutz
            </Link>
            <Link to={dePaths.shipping} className="hover:text-[var(--primary)]">
              Versand
            </Link>
            <Link to={dePaths.contact} className="hover:text-[var(--primary)]">
              Support
            </Link>
          </div>
        </div>
