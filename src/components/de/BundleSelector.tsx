import { deBundles, eur, DE_SENSOR_DAYS, type DeBundleId } from "@/lib/locale/de/bundles";

export function DeBundleSelector({
  selected,
  onSelect,
}: {
  selected: DeBundleId;
  onSelect: (id: DeBundleId) => void;
}) {
  return (
    <div className="space-y-3">
      {deBundles.map((b) => {
        const active = b.id === selected;
        const unitWord = b.months === 1 ? "Einheit" : "Einheiten";
        const label = (
          <>
            <strong className="font-semibold">
              {b.months} {unitWord} = {b.sensors} Sensoren
            </strong>{" "}
            <span className={active ? "text-white/85" : "text-[var(--ink)]/55"}>
              ({DE_SENSOR_DAYS} Tage je Sensor) ({b.monitoringDays} Tage gesamt)
            </span>
          </>
        );
        return (
