import { mxBundles, mxn, MX_SENSOR_DAYS, type MxBundleId } from "@/lib/locale/mx/bundles";

export function MxBundleSelector({
  selected,
  onSelect,
}: {
  selected: MxBundleId;
  onSelect: (id: MxBundleId) => void;
}) {
  return (
    <div className="space-y-3">
      {mxBundles.map((b) => {
        const active = b.id === selected;
        const unitWord = b.months === 1 ? "unidad" : "unidades";
        const label = (
          <>
            <strong className="font-semibold">
              {b.months} {unitWord} = {b.sensors} sensores
            </strong>{" "}
            <span className={active ? "text-white/85" : "text-[var(--ink)]/55"}>
              ({MX_SENSOR_DAYS} días por sensor) ({b.monitoringDays} días en total)
            </span>
          </>
        );
        return (
